...
export default function SumireLorePage() {
  return (
    <div className="min-h-screen bg-black md:bg-[#050408] text-white selection:bg-[#8a60c2] selection:text-white overflow-x-hidden scroll-smooth">
      ...
      <div className="fixed top-[10%] left-[-10%] w-[50vw] h-[50vh] bg-[#8a60c2]/10 rounded-full blur-[120px] pointer-events-none hidden md:block" />
      <FloatingParticles />
      ...
      {/* Шапка (Hero Section) */}
      <header className="relative min-h-[100dvh] flex flex-col items-center justify-center text-center px-4 py-20 z-10">
        <motion.div
      ...
      <main className="relative z-10 w-full pb-20">
        ...
        {/* ИЗМЕНЕНИЕ: Идеальное растворение картинки с помощью CSS Mask */}
        {/* Картинка сама становится прозрачной, поэтому фоновое свечение не перекрывается */}
        <motion.div 
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 1.2 }}
          className="relative w-full max-w-6xl mx-auto aspect-[16/9] md:aspect-[21/9] mb-4 md:mb-8"
        >
          {/* Вертикальная маска (Плавный переход сверху и очень долгий снизу) */}
          <div 
          ...
          >
            {/* Горизонтальная маска (Плавный переход слева и справа) */}
            <div 
              className="absolute inset-0 w-full h-full"
              style={{
        ...
        {/* Заголовок тома (Наплывает на градиент картинки) */}
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 1, delay: 0.2 }}
          className="relative z-10 text-center mb-16 md:mb-20 px-4 -mt-20 md:-mt-32"
        >
      ...
      {/* Футер */}
      <footer className="relative py-20 md:py-24 border-t border-white/5 text-center z-10 mt-10">
        <motion.div
          initial={{ opacity: 0 }}
